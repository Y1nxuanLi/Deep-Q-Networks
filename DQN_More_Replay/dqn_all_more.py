...
def evaluation(reward):
    ...
    while steps < 100000:  # Run for 1000 steps or until termination
        # Preprocess observation and select action
        ...
        if steps == 0 or terminated or truncated:
            action = 1  # ensures that the game is properly started with the "fire" action.
    ...
    env_display.close()
    print(f"Evaluation completed. Total rewards over {steps} steps: {total_rewards}")
...
if __name__ == "__main__":
    ...
    Deep_Q_Learning(env, buffer_size=250_000, nb_epochs=5_000_000, exploration_steps=100_000, replay_start_size=5_000, device='cuda', C=1_000)
